...
class QD_GCN(nn.Module):
    ...
    def forward(self, node_input, att_input, adj, Fadj, feat, training=True):
        ...
        return torch.sigmoid(model) ,model
...
class CS_GCN(nn.Module):
    ...
    def forward(self, node_input, adj, feat, training=True):
        model1=self.graph_encoder1(feat,adj)+ self.self_GE1(feat) # H_G_0
        model2 = self.structure_encoder1(node_input, adj) + self.self_SE1(node_input) # H_Q_0

        model= torch.cat([model1,model2],2) # H_FF_0
        model=self.cnd1(model)
        model = self.bn1(model.transpose(-1, -2)).transpose(-1, -2)
        model = F.relu(model)
        model = F.dropout(model, self.dropout, training=training)
        ...
        model1 = F.relu(model1)
        model1 = F.dropout(model1, self.dropout, training=training)
        model1=self.graph_encoder2(model1,adj)+ self.self_GE2(model1) # H_G_1
        ...
        model2 = self.structure_encoder2(model, adj) + self.self_SE2(model_SE) # H_Q_1
        ...
        model = self.bn2(model.transpose(-1, -2)).transpose(-1, -2)
        model = F.relu(model)
        model = F.dropout(model, self.dropout, training=training) # H_FF_1
        ...
        model1 = F.relu(model1)
        model1 = F.dropout(model1, self.dropout, training=training)
        model1=self.graph_encoder3(model1,adj)+ self.self_GE3(model1) # H_G_2

        model_SE = self.bn_SE2(model2.transpose(-1, -2)).transpose(-1, -2)
        model_SE = F.relu(model_SE)
        model_SE = F.dropout(model_SE, self.dropout, training=training)
        model2 = self.structure_encoder3(model, adj) + self.self_SE3(model_SE) # H_Q_2


        model= torch.cat([model1,model2],2)
        model=self.cnd3(model) # H_FF_2
