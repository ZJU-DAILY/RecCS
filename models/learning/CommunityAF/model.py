...
class coreModel(nn.Module):
    ...
    def forward(self, attr, batch_data):
        ...
        batch_next_onehot_np = np.array(batch_next_onehot)
        x_deq = torch.tensor(batch_next_onehot_np, dtype=torch.float).to(self.device)
        repeat = x_deq.size(0)
            ...
            if self.args.rankingloss:
                ...
                if batch_labels[i] == 1:
                    for j in range(len(batch_neg[neg_idx])):
                        ...
                        if attr != None:
                            assert h.shape[0] == len(batch_com[i]) + len([batch_neg[neg_idx][j]]) + len(batch_neg_neighbors[neg_idx][j])
                            concate = (batch_com[i] + [batch_neg[neg_idx][j]]) + sorted(batch_neg_neighbors[neg_idx][j])
                            h += self.attr_embedding(attr[concate])
                        neg_now = self.input_mapping(h)

                        negcom.append(len(embs))
                        embs.append(torch.cat(
                            (self.pool(torch.unsqueeze(neg_now, 0)).view(-1), neg_now.mean(dim=0))))
