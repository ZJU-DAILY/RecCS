...
class Graph:
    ...
    def __init__(self, nxg):
        self.neighbors, self.n_nodes, self.adj_mat, self.nodes = self._init_from_graph(nxg)

    @staticmethod
    def _init_from_graph(nxg):
        edges = np.array([[edge[0], edge[1]] for edge in nxg.edges()])
        ...
        for u, v in edges:
            ...
            nodes.add(u)
            nodes.add(v)
        adj_mat = sp.csr_matrix((np.ones(len(edges)), edges.T), shape=(N, N))
        adj_mat += adj_mat.T
        return neighbors, N, adj_mat,nodes
