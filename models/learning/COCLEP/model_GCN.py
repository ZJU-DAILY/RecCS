...
class Contra(nn.Module):
    ...
    def forward(self, h, h_aug, tau, train, alpha, lam, edge_index, valid = False):
        if self.training == False and valid == False:
            return h
...
class ConRC(nn.Module):
    ...
    def compute_loss(self, train, valid = False):
        loss = None
        ...
        if loss is None:
            loss = self.contra(h_, h_auge, self.tau, (q, pos), self.alpha, self.lam, edge_index, valid)
        else:
            loss = loss + self.contra(h_, h_auge, self.tau, (q, pos), self.alpha, self.lam, edge_index, valid)
